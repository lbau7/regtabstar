#' Regression tables for lmerModLmerTest models
#'
#' regtab method for models of class \code{lmerModLmerTest}.
#'
...
#' regtab(bdf.lmerMod)
regtab.lmerModLmerTest <- function(mod, format = "latex",
                                   style_options = list(), ci = TRUE,
                                   ci_level = 0.95, se = FALSE,
                                   teststatistic = FALSE, df = TRUE,
                                   pval = TRUE, intercept = FALSE,
                                   caption = NULL, n_caption = TRUE,
                                   rowlabs = NULL,
  ...
  if (df) coefsm[, "df"] <- round(coefsm[, "df"], 1)
  if (pval) coefsm[highsig, ncol(coefsm)] <- "<0.001"

  if (addref) {
    modframe <- mod@frame[, -c(1, ncol(mod@frame)- length(mod@flist) +
        1:ncol(mod@frame)), drop = FALSE]
